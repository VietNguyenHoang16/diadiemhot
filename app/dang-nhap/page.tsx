'use client';

import { signIn, useSession } from 'next-auth/react';
import { useRouter } from 'next/navigation';
import { useEffect, useState } from 'react';
import Footer from '@/app/components/Footer';

export default function Login() {
  const router = useRouter();
  const { data: session, status } = useSession();
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    if (status === 'authenticated') {
      router.push('/dashboard');
    } else if (status === 'unauthenticated') {
      setLoading(false);
    }
  }, [status, router]);

  const handleGoogleLogin = () => {
    signIn('google', { callbackUrl: '/dashboard' });
  };

  if (loading || status === 'loading') {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="w-8 h-8 border-4 border-slate-200 border-t-[#bb0012] rounded-full animate-spin"></div>
      </div>
    );
  }

  return (
    <main className="min-h-screen bg-white">
      {/* Header */}
      <header className="fixed top-0 w-full z-50 bg-white/80 backdrop-blur-xl shadow-sm">
        <div className="bg-[#00173a] text-white py-2 px-8 overflow-hidden whitespace-nowrap">
          <div className="flex items-center gap-4 text-xs tracking-[0.2em] font-bold uppercase">
            <span className="bg-[#bb0012] px-2 py-0.5 rounded-sm">Nổi Bật</span>
            <p className="animate-pulse">Phát Hiện Quán Cà Phê Mới Tại Trung Tâm • Công Viên Xanh Vừa Khai Trương • Top Nhà Hàng Tháng 10 ...</p>
          </div>
        </div>
        <nav className="flex justify-between items-center px-8 h-16 w-full max-w-screen-2xl mx-auto">
          <div className="text-2xl font-black tracking-tighter text-[#00173a] uppercase">
            Địa Điểm Hot
          </div>
          <div className="hidden xl:flex items-center space-x-6 text-sm font-semibold">
            <a className="text-slate-600 hover:text-[#bb0012]" href="/">Khám Phá</a>
            <a className="text-slate-600 hover:text-[#bb0012]" href="/blog">Blog</a>
            <a className="text-slate-600 hover:text-[#bb0012]" href="/du-lich">Du Lịch</a>
            <a className="text-slate-600 hover:text-[#bb0012]" href="/phong-cach">Xếp Hạng</a>
          </div>
          <div className="flex items-center gap-4">
            <a href="/" className="text-sm font-medium text-[#bb0012] hover:underline">Về trang chủ</a>
          </div>
        </nav>
      </header>

      <div className="min-h-screen flex items-center justify-center px-4 py-32">
        <div className="w-full max-w-md">
          <div className="bg-white shadow-lg border border-slate-200 rounded-2xl p-8">
            <h1 className="text-2xl font-black text-[#00173a] uppercase text-center mb-2">
              Đăng Nhập
            </h1>
            <p className="text-center text-slate-600 mb-6">
              Đăng nhập bằng Gmail để quản lý doanh nghiệp của bạn
            </p>

            {/* Google Login */}
            <button 
              onClick={handleGoogleLogin}
              className="w-full py-4 border-2 border-slate-300 rounded-xl font-bold flex items-center justify-center gap-3 hover:bg-slate-50 transition-colors text-lg"
            >
              <svg className="w-6 h-6" viewBox="0 0 24 24">
                <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
                <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
                <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.38 8.55 1 10.22 1 12s.38 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
                <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
              </svg>
              Đăng nhập bằng Gmail
            </button>

            <p className="text-center text-sm text-slate-500 mt-6">
              Chưa có tài khoản? <a href="mailto:anivia161@gmail.com" className="text-[#bb0012] font-medium hover:underline">Liên hệ để đăng ký</a> · <span className="text-slate-400">0901.400.248</span>
            </p>
          </div>

          <p className="text-center text-sm text-slate-500 mt-6">
            <a href="/" className="text-[#bb0012] font-medium hover:underline">← Về trang chủ</a>
          </p>
        </div>
      </div>

      <Footer />
    </main>
  );
}